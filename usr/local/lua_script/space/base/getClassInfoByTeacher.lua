...
local sel_class = "SELECT DISTINCT t1.class_id as class_id,class_name FROM t_base_class AS t1 INNER JOIN t_base_class_subject AS t2 on t1.class_id = t2.class_id INNER JOIN t_base_term t3 on t3.XQ_ID = t2.XQ_ID and t3.SFDQXQ=1 WHERE teacher_id = "..ngx.quote_sql_str(person_id);

...
local student_res= db:query("SELECT student_id,student_name,'0' as userPhoto FROM t_base_student WHERE B_USE=1 AND CLASS_ID="..class_id.." limit "..limit)
local student_info = student_res

local t_r = db:query("SELECT count(*) as totalRow FROM t_base_student WHERE B_USE=1 AND CLASS_ID="..class_id)
...
if person_id and string.len(person_id)>0 and identity_id and string.len(identity_id)>0 then
    --查询关注情况

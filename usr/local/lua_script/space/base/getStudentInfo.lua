...
local sel_student= "SELECT student_id,student_name FROM t_base_student WHERE class_id = (SELECT class_id from t_base_student WHERE student_id = "..student_id..")  AND student_id <> "..student_id.." AND B_USE = 1 LIMIT "..limit;
local total_sql = "SELECT count(*) as totalRow FROM t_base_student WHERE class_id = (SELECT class_id from t_base_student WHERE student_id = "..student_id..") AND student_id <> "..student_id.." AND B_USE = 1";

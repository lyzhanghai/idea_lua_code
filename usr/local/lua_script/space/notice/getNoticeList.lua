...
local thumb_flag = ngx.var.arg_thumb_flag
local sphinx_flag = ngx.var.arg_sphinx_flag --title,overview,content,任意组合
local notice_type = ngx.var.arg_notice_type --

if not page_size or len(page_size) == 0 or 
...
if not sphinx_flag or len(sphinx_flag) == 0 then
    sphinx_flag = "title"
end
...
local register_id_filter = ""
if register_id and len(register_id) > 0 then
    register_id_filter = "filter=register_id,"..register_id..";"
end
...
if title and len(title) > 0 then
    ...
    title_filter = "mode=extended2;@("..sphinx_flag..") "..title..";"
end
...
local sort_type_sql = ""
if sort_type and sort_type == "2" then
    sort_type_sql = "sort=attr_desc:view_count;"
else
    sort_type_sql = "sort=attr_desc:ts;"
end

local thumb_flag_sql = ""
if thumb_flag and thumb_flag == "1" then
    thumb_flag_sql = "!filter=thumbnail,0;"
end

local notice_type_filter = ""
if notice_type and len(notice_type) > 0 then
    notice_type_filter = "filter=notice_type,"..notice_type..";"
end

local offset = page_size*page_number - page_size
local limit = page_size
local ssql = "SELECT SQL_NO_CACHE ID FROM T_SOCIAL_NOTICE_SPHINXSE WHERE QUERY= '"..title_filter..org_ids_filter..category_id_filter..stage_id_filter..subject_id_filter..register_id_filter..notice_type_filter..
    "filter=b_delete,0;"..sort_type_sql..thumb_flag_sql.."offset="..offset..";limit="..limit..";';SHOW ENGINE SPHINX STATUS;"
...
for _, v in ipairs(r) do
    ...
    if hr and hr[1] ~= "ok" and hr[1] ~= "not_find" then
        ...
        if category_id and len(category_id)>0 and category_id ~= "0" and category_id ~= "-1" then

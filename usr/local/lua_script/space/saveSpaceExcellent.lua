...
local getParams = function()
    ...
    local record_id = args["record_id"]
    ...
    return org_id,org_type,record_id,province_id,city_id,district_id,school_id,class_id,identity_id
end

local function save(record_id,org_id,org_type,province_id,city_id,district_id,school_id,class_id,identityid)
...
local function saveExcellent()
    ...
    local sta,org_id,org_type,record_id,province_id,city_id,district_id,school_id,class_id,identity_id = pcall(getParams);
    if not sta then
	  resResult.info="参数错误！"
    ...
    local result,err = save(record_id,org_id,org_type,province_id,city_id,district_id,school_id,class_id,identity_id)
    if not result then
	  resResult.info="保存出错！"
	  return cjson.encode(resResult)
    end
